...
function node(id){if(!nodes.has(id))nodes.set(id,{id,value:'',innerHTML:'',textContent:'',style:{},querySelector:()=>null,querySelectorAll:()=>[],insertAdjacentHTML(_p,t){this.innerHTML+=t},append(){},focus(){},setSelectionRange(){}});return nodes.get(id)}
const ctx={console,URL,AbortController,Set,Number,Date,JSON,Array,Error,Promise,document:{getElementById:node,querySelectorAll:()=>[],addEventListener:()=>{},activeElement:null,hidden:false,createElement:()=>({})},localStorage:{getItem:k=>{if(storageBlocked)throw Error();return data.get(k)||null},setItem:(k,v)=>{if(storageBlocked)throw Error();data.set(k,v)},removeItem:k=>data.delete(k)},alert:x=>alerts.push(x),location:{pathname:'/'},history:{pushState:(_a,_b,path)=>ctx.location.pathname=path},window:{},setTimeout:()=>1,clearTimeout:()=>{},setInterval:()=>1,fetch:async()=>({ok:true,json:async()=>({data:[],sources:[],status:'UNAVAILABLE',authenticated:false})})};
